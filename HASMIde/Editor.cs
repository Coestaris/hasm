...
namespace HASM
{
	public partial class Editor : Form
	{
		...
		private void AddTab(string path)
		{
			foreach (TextEditor page in tabControl1.TabPages)
			{
				if (page.Path.CompareAsPath(path))
				{
					tabControl1.SelectedTab = page;
					return;
				}
		...
		private void treeView1_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
		{
			...
			if (fnode.isRoot) toolStripMenuItem_node_name.Text = "[root]";
					...
					$"{(fnode.IsDir ? "[" : "")}" +
										$"{Formatter.MakeRelative(fnode.AbsolutePath, workingFolder.Path + PlatformSpecific.NameSeparator)}" +
					$"{(fnode.IsDir ? "]" : "")}";
		...
		private void treeView1_DragDrop(object sender, DragEventArgs e)
		{
			...
			string newPath = PlatformSpecific.CombinePath(destinationNode.AbsolutePath, Path.GetFileName(sourceNode.AbsolutePath));
